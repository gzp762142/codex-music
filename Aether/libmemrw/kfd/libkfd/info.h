...
void info_init(struct kfd* kfd)
{
    ...
    /*
     * Match on the "Darwin Kernel Version <major>.<minor>.<patch>" prefix only.
     *
     * The upstream table stores a full banner, so a point release such as
     * 22.5.0 versus 22.5.1 misses the entry it belongs to and the kernel
     * address table is never selected. Comparing a fixed prefix keeps the
     * per-line entries usable across point releases; the offsets that matter
     * here (proc / task within one Darwin major) do not move between them.
     */
    ...
    print_failure("no version table entry for: %s", kern_version);
    print_failure("add one to kern_versions[] in libkfd/info/dynamic_info.h");
