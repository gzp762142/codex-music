...
typedef enum {
    ...
    /// 前置不成立：内核层未就绪 / PA→KVA 换算表建立失败 / 页大小取不到。
    /// **不是**"没有页表"，是"还没资格问这个问题"。
    ///
    /// 换算表**按需自建**（本模块内部调 `km_phystokv_ensure()`，与 KernelPhysMap
    /// 同口径）—— 不需要用户先去点「Slide」。2026-09-26 之前这里要求先点 Slide，
    /// 真机症状是「只点建窗 → 遍历中断：PA→KVA 换算不可用」，而能力本来就有。
    KM_PW_NOT_READY,
